...
//		TEST_CASE_DESCRIBE(testMatrixInvert, "Inverts a matrix");
	...
	void testMatrixMult()
	{
		...
		for( int i = 0; i < 4; ++i )
		{
			for( int j = 0; j < 4; ++j )
			{
				ASSERT_EQUALS_EPSILON( mat0.ToD3D()->m[ i ][ j ], matR.ToD3D()->m[ i ][ j ], 0.01f );
			}
		}
	}
	void testMatrixAdd()
