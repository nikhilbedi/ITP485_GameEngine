...
namespace ITP485
{
	...
	void GameWorld::LoadPointLight(const std::string pointLightName, const minIni& ini)
	{
		...
		// Obtain Point light index -- 0, 1, 2, 3 exist (max 4 points)
		int pointLightIndex = atoi(pointLightName.substr(pointLightName.size() - 1, pointLightName.size() - 1).c_str());

		// Add to the gameworld
		mLightingConstants.mPointLights[pointLightIndex] = pointLight;
	}
